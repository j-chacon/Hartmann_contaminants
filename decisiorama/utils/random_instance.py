...
class Multinomial(RandomNumber):
    '''Class to create a multinomial random value'''
    def __init__(self, val, prob, n=None):
        self.val = val
        self.prob = prob
        self.n = n
        
    def get(self, n):
        _t = np.argmax(np.random.multinomial(1, self.prob, size=n), axis=1)
        out = [self.val[i] for i in _t]
        return out
...
#     print(Multinomial([1,2,3], [0.2, 0.3, 0.8]).get(10))
